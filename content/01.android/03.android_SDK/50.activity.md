...
スリープを無効にするためには、 `Activity` から参照できる `window` インスタンスに対して、 `FLAG_KEEP_SCREEN_ON` のフラグを追加する。
...
用が済んだらスリープ無効設定を解除する必要がある。
設定をクリアするためには、同様に `window` インスタンスに対して `FLAG_KEEP_SCREEN_ON` のフラグをクリアする。
...
* [デバイスの起動状態を維持する | Background work | Android Developers](https://developer.android.com/develop/background-work/background-tasks/scheduling/wakelock)
* [【Android/Kotlin】Kotlinのコードで画面のスリープを無効にする - 映画と旅行とエンジニア](https://wakky.tech/keep-screen-on-android-cup-noodle-timer6/#google_vignette)


# ソフトキーボードを閉じる実装

`InputMethodManager` にアクセスし、キーボードを閉じるよう `hideSoftInputFromWindow` で要求する。

```kotlin
// システムサービスからInputMethodManagerを取得
val inputMethodManager = getSystemService(INPUT_METHOD_SERVICE) as? InputMethodManager

// 現在フォーカスが当たっているView（v）のwindowTokenを渡してキーボードを閉じる
...
例えば、Androidで方向キー（十字キー）のキーイベントを疑似的に発行する場合、以下のキーコードを使用する。

* 方向キー下：[KEYCODE_DPAD_DOWN](https://developer.android.com/reference/android/view/KeyEvent#KEYCODE_DPAD_DOWN)
* 方向キー上：[KEYCODE_DPAD_UP](https://www.google.com/search?q=https://developer.github.com/reference/android/view/KeyEvent%23KEYCODE_DPAD_UP)

実装はシンプルで、 `KeyEvent` インスタンスを生成し、Activity等の `dispatchKeyEvent()` に投げるだけ。
...
ここで気をつけなければならないのは、 `KeyEvent` のコンストラクタに渡している第1引数（アクション）に `KeyEvent.ACTION_DOWN` を指定すること。

`ACTION_DOWN` はキーが押され始めた瞬間、 `ACTION_UP` はキーから指が離れた瞬間を表すイベント。
しかし、第1引数に `ACTION_UP` だけを指定してイベントを発行しても、OS側には処理を無視されてしまう。

これはOSの立場からしてみれば、「押されてもいないのに、いきなり離されたとはどういうことだ？ 不正な状態なので無視しておこう」
という形で処理が弾かれてしまうため。

そのため、どうしても `ACTION_UP` のタイミングで何かしらの制御を行いたい場合であっても、
まずは対となる `ACTION_DOWN` のイベントを先に発行し、その直後に `ACTION_UP` を発行するという一連の手順を踏む必要がある。
`ACTION_DOWN` ひとつで事足りるユースケースであれば、実装は非常に簡単だ。

## 参考

* [Android備忘録：BACKキーやHOMEボタンのイベント発行方法 - Sisyou's Blog](https://sisyoukum.blogspot.com/2014/02/androidbackhome.html)